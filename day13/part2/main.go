...
func editDistance(value [][]rune, reflectionPoint int) int {
	...
	for i := 0; i < elemsToCheck; i++ {
		for j := 0; j < len(value[leftOfPoint]); j++ {
			if value[leftPtr][j] != value[rightPtr][j] {
				numDiffs++
			}
		}
